...
inside functions to avoid circular imports with `agents.main`. Callers can import
these helpers directly from `agents.rag`.
"""
...
# Config defaults (can be overridden by env in agents.main)
...
def add_documents(scope: Literal["global", "private"], user_id: Optional[str], docs: List[Document]) -> int:
    ...
    This function imports `doc_vs` lazily from `agents.main` to avoid
    circular import issues.
    """
    # Lazy import to avoid circular dependency on agents.main
