...
Run as: python -m rag.daemons.url_watcher
Or from repo root:
    PYTHONPATH=backend/backend_app python backend/backend_app/rag/daemons/url_watcher.py
...
# Import helpers from the agents package. This assumes the script is executed
# with Python path such that `agents` is importable (e.g. when run from
# backend/backend_app or with PYTHONPATH set accordingly).
try:
    from backend.backend_app.agents.backup.rag import fetch_url_text, add_documents, sha256_text
    from agents.main import mark_source_status, COLLECTION_DOCS
...
def delete_vectors_for_source(conn, source_id: str, collection_table: str) -> int:
    ...
    try:
        from agents.main import doc_vs  # type: ignore
